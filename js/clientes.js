...
const DELETE_CLIENT_ENDPOINT = (id) => `${BASE_API_URL}/client/${id}`;
...
function renderClientRow(client) {
    ...
    const nombreCompleto = [client.nombre, client.primerApellido, client.segundoApellido]
        .filter(Boolean).join(' ');
    const fechaRegistro = new Date(client.createdAt || Date.now()).toLocaleDateString();
    
    tr.innerHTML = `
    <td>${client.idCliente ?? client.id ?? ''}</td>
    <td>${nombreCompleto}</td>
    <td>${client.numeroTelefono || 'N/A'}</td>
    <td><span class="badge bg-${client.creditoActivo ? 'success' : 'secondary'}">${client.creditoActivo ? 'Activo' : 'Inactivo'}</span></td>
    <td>${fechaRegistro}</td>
    <td>
        <button class="btn btn-sm btn-outline-primary btn-edit me-1" data-id="${client.idCliente || client.id}">
            <i class="bi bi-pencil"></i>
        </button>
        <button class="btn btn-sm btn-outline-danger btn-delete" data-id="${client.idCliente || client.id}">
...
function filterClients() {
    ...
    let filtered = currentClients.filter(client => {
        ...
        const matchesSearch = !searchTerm || 
            ...
            (client.numeroTelefono?.includes(searchTerm)) ||
            (client.id?.toString().includes(searchTerm));
        
        const matchesCredito = !creditoFilter || 
...
async function deleteClient(id) {
    const token = localStorage.getItem('authToken');
    const res = await fetch(DELETE_CLIENT_ENDPOINT(id), {
        method: 'DELETE',
        headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json'
...
function initModalLogic() {
    ...
    document.getElementById('formClient').addEventListener('submit', async (e) => {
        ...
        const payload = {
            ...
            primerApellido: document.getElementById('inputClientPrimerApellido').value.trim(),
            segundoApellido: document.getElementById('inputClientSegundoApellido').value.trim(),
            numeroTelefono: document.getElementById('inputClientTelefono').value.trim(),
        ...
        try {
            ...
            if (editingClientId) {
                ...
                displayMessage && displayMessage('Cliente actualizado correctamente.');
            } else {
    ...
    document.querySelector('#clientsTable tbody').addEventListener('click', async (e) => {
        ...
        if (btnEdit) {
            ...
            if (client) {
                ...
                document.getElementById('inputClientNombre').value = client.nombre || '';
                document.getElementById('inputClientPrimerApellido').value = client.primerApellido || '';
                document.getElementById('inputClientSegundoApellido').value = client.segundoApellido || '';
                document.getElementById('inputClientTelefono').value = client.numeroTelefono || '';
                document.getElementById('inputClientCredito').checked = client.creditoActivo || false;
                modalInstance.show();
            }
        }
        ...
        if (btnDelete) {
            ...
            if (confirm('¿Estás seguro de que deseas eliminar este cliente?')) {
                try {
                    ...
                    displayMessage && displayMessage('Cliente eliminado correctamente.');
                    await loadClients();
                } catch (err) {
                    console.error('deleteClient error', err);
                    displayError(`No se pudo eliminar el cliente: ${err.message || err}`);
                }
            }
        }
    });
